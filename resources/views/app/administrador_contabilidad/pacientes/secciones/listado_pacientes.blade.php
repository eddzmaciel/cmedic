...
<!--  DATO NUEVO -->
<form action="/api_cont/pacientes/" id="frmNuevoDato" method="POST" class="smart-form" style="display:none;">
...
<script type="text/javascript">
	...
	_g.dao = {
			...
			getModificarDatos :function(id){
						$.ajax({
							url: '/api_cont/info_pacientes/'+id,
							type: 'GET',
							dataType: 'json',			
						}).done(function(data){
		...
		var functionOperacionesForm = function() {
				...
				$('#btnAceptarEditar').on('click',function (e){
						...
						if($("#frmEditarDatos").valid() == true && $("#inpt1").val() != ''){
							$.ajax({
								url: '/api_cont/pacientes/'+$("#inpt1").val(),
			...
			/*	$('#btnAgregarNuevo').on('click',function (e){
		               e.preventDefault();
		               $.ajax({
		                       type:"GET",
		                       url:'/webs/mail/',
		                       ...
		                       success: function(data) {
		                       			console.log(data);
		                               _gen.notificacion_min('&Eacute;xito', 'Se envi&oacute; la notificaci&oacute;n exitosamente.',1);
		                           }, 
		                        error: function(data) { 
		                        	console.log(data);
		                        	_gen.notificacion_min('Error', data.responseText,4)
