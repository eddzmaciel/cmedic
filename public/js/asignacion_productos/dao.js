_g.dao = {
	getProductosAsignados :function(id_proveedor){
		$.ajax({
			url: '/api_inv/rel_productos_proveedor/'+id_proveedor,
			type: 'GET',
			dataType: 'json',
		}).done(function(data){
	...
	getProductosActivos : function(){
		$.ajax({
			...
			$.get('api_inv/rel_productos_proveedor/'+$("#id_proveedores").val(), function(data){
				if(data!=""){
					$.each(data, function(i) {
