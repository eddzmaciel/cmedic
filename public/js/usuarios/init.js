...
jQuery(function($) {
	$('#btnEditar').click(function(e){
		...
		if(_g.currentDates['id'] != null  && _g.currentDates['id'] != ''){
			window.location.href='/#admin/usuarios/editar/'+_g.currentDates['id'];			
		}else{
			_gen.notificacion_min('Aviso', 'Es necesario seleccionar un registro',4);
		}
	});
	$('#btnEliminar').click(function(e){
		e.preventDefault();
		if(_g.currentDates['id'] != null  && _g.currentDates['id'] != ''){
			_g.dao.getEliminarUsuarios(_g.currentDates['id']);		
		}else{
			_gen.notificacion_min('Aviso', 'Es necesario seleccionar un registro',4);
		}
	});
